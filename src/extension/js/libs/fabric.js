...
(function (global) {
  ...
  extend(fabric, {
    parseFontDeclaration: function (value, oStyle) {
      var fontDeclaration = "(normal|italic)?\\s*(normal|small-caps)?\\s*" + "(normal|bold|bolder|lighter|100|200|300|400|500|600|700|800|900)?\\s*(" + fabric.reNum + "(?:px|cm|mm|em|pt|pc|in)*)(?:\\/(normal|" + fabric.reNum + "))?\\s+(.*)",
  ...
  fabric.Canvas = fabric.util.createClass(fabric.StaticCanvas, {
    ...
    _scaleObjectEqually: function (localMouse, target, transform) {
      var dist = localMouse.y + localMouse.x,
        strokeWidth = target.stroke ? target.strokeWidth : 0,
        lastDist = (target.height + strokeWidth / 2) * transform.original.scaleY + (target.width + strokeWidth / 2) * transform.original.scaleX;
  ...
  fabric.Path = fabric.util.createClass(fabric.Object, {
    ...
    _render: function (ctx) {
      var current, previous = null,
        ...
        controlX = 0,
        controlY = 0,
        tempX, tempY, tempControlX, tempControlY, l = -this.pathOffset.x,
        t = -this.pathOffset.y;
      if (this.group && this.group.type === "path-group") {
        l = 0;
        t = 0
    ...
    _parseDimensions: function () {
      var aX = [],
        ...
        controlX = 0,
        controlY = 0,
        tempX, tempY, tempControlX, tempControlY, bounds;
      for (var i = 0, len = this.path.length; i < len; ++i) {
        current = this.path[i];
        switch (current[0]) {
        case "l":
  ...
  fabric.Text = fabric.util.createClass(fabric.Object, {
    ...
    _getSVGLeftTopOffsets: function (textLines) {
      var lineTop = this.useNative ? this.fontSize * this.lineHeight : -this._fontAscent - this._fontAscent / 5 * this.lineHeight,
        ...
        textTop = this.useNative ? this.fontSize * this.lineHeight - .25 * this.fontSize : this.height / 2 - textLines.length * this.fontSize - this._totalLineHeight;
      return {
  ...
  fabric.IText = fabric.util.createClass(fabric.Text, fabric.Observable, {
    ...
    _getCursorBoundaries: function (chars, typeOfBoundaries) {
      var cursorLocation = this.get2DCursorLocation(),
        ...
        offsets = this._getCursorBoundariesOffsets(chars, typeOfBoundaries, cursorLocation, textLines);
      return {
        left: left,
        top: top,
    ...
    _getCursorBoundariesOffsets: function (chars, typeOfBoundaries, cursorLocation, textLines) {
      var lineLeftOffset = 0,
        ...
        charIndex = 0,
        leftOffset = 0,
        topOffset = typeOfBoundaries === "cursor" ? this._getHeightOfLine(this.ctx, 0) - this.getCurrentCharFontSize(cursorLocation.lineIndex, cursorLocation.charIndex) : 0;
...
fabric.util.object.extend(fabric.IText.prototype, {
  ...
  getDownCursorOffset: function (e, isRight) {
    var selectionProp = isRight ? this.selectionEnd : this.selectionStart,
      textLines = this.text.split(this._reNewline),
      _char, lineLeftOffset, textBeforeCursor = this.text.slice(0, selectionProp),
  ...
  getUpCursorOffset: function (e, isRight) {
    ...
    var textBeforeCursor = this.text.slice(0, selectionProp),
      ...
      textLines = this.text.split(this._reNewline),
      _char, widthOfSameLineBeforeCursor = this._getWidthOfLine(this.ctx, cursorLocation.lineIndex, textLines),
      lineLeftOffset = this._getLineLeftOffset(widthOfSameLineBeforeCursor),
